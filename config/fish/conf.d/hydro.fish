...
set --universal _hydro_git _hydro_git_$fish_pid
...
function _hydro_pwd --on-variable PWD --on-variable hydro_ignored_git_paths --on-variable fish_prompt_pwd_dir_length
    ...
    if set --query git_root[1] && ! contains -- $git_root $hydro_ignored_git_paths
        ...
        set --universal _hydro_skip_git_prompt
    end

    set --universal _hydro_pwd (
...
function _hydro_postexec --on-event fish_postexec
    ...
    set --universal _hydro_status "$_hydro_newline$_hydro_color_prompt$hydro_symbol_prompt"

    for code in $last_status
        if test $code -ne 0
            set --universal _hydro_status "$_hydro_color_error| "(echo $last_status)" $_hydro_newline$_hydro_color_prompt$_hydro_color_error$hydro_symbol_prompt"
            break
        end
    end
    ...
    set --universal _hydro_cmd_duration "$out "
end
...
function _hydro_prompt --on-event fish_prompt
    ...
    set --universal _hydro_last_pid $last_pid
end
...
set --universal hydro_color_normal (set_color normal)

for color in hydro_color_{pwd,git,error,prompt,duration,start}
    function $color --on-variable $color --inherit-variable color
        set --query $color && set --universal _$color (set_color $$color)
    end && $color
end

function hydro_multiline --on-variable hydro_multiline
    if test "$hydro_multiline" = true
        set --universal _hydro_newline "\n"
    else
        set --universal _hydro_newline ""
    end
end && hydro_multiline

set --universal hydro_color_error $fish_color_error
set --universal hydro_symbol_prompt ''
set --universal hydro_symbol_git_dirty •
set --universal hydro_symbol_git_ahead ↑
set --universal hydro_symbol_git_behind ↓
set --universal hydro_multiline false
set --universal hydro_cmd_duration_threshold 1000
